...
### How does CNML relate to verifiable credentials and registered-entity trust models?

CNML interoperates with W3C Verifiable Credentials: certificates are expressible in the VC shape, and the verifier understands Bitstring Status List revocation alongside certificate revocation lists. The trust model is different in kind and, for legal metrology, stronger. Registered-entity technologies establish an issuer's standing through recognition credentials that a verifier fetches and walks online. CNML establishes signing authority through cryptographic delegation: threshold roots that cannot be forged by any single compromise, mandatory transparency that makes covert issuance detectable, revocation that propagates to every artifact bound to a withdrawn state, scope enforced by the verification mathematics, and offline verification from an anchor bundle. The full comparison, including where each model fits and how they compose, is in [CNML and verifiable credentials](/docs/concepts/cnml-and-verifiable-credentials).
