...
The current pipeline order is: XML well-formedness, schema validity, signature validity, dimensional co-signatures, scope enforcement, CRL status, evaluation-report binding, timestamp anchoring, and transparency-log inclusion. The order matters because earlier checks short-circuit later ones. A file that is not well-formed XML cannot be schema-validated. A file that fails schema validation cannot have its signature checked meaningfully. Adding a new check requires one new file under `checks/` and one entry in the `CHECKS` array. The verification pipeline is developed further in [Verification pipeline](/docs/implementation/verification-pipeline).
