...
// ─── quorum extension reader (round 4) ───────────────────────────

import { readFileSync } from "node:fs";
import { fileURLToPath } from "node:url";
import path from "node:path";
import { readQuorumFromCert } from "../scope.ts";

const FIXTURES = path.join(path.dirname(fileURLToPath(import.meta.url)), "__fixtures__");

test("quorum parameters read off a threshold delegation cert", async () => {
  // Generated by openssl: SEQUENCE { INTEGER 5, INTEGER 7 } under the
  // quorum OID, plus the scope fixture for contrast.
  const pem = readFileSync(path.join(FIXTURES, "quorum-cert.pem"), "utf8");
  const q = await readQuorumFromCert(pem);
  assert.deepEqual(q, { t: 5, n: 7 });
});

test("certs without the quorum extension read as null", async () => {
  const pem = readFileSync(path.join(FIXTURES, "tester-scope-cert.pem"), "utf8");
  assert.equal(await readQuorumFromCert(pem), null);
});
